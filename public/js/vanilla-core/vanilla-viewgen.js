...
export function createMainView(user, onRouteChange) {
    ...
    return div({ id: "body-wrapper", className: "bg-gray-100 flex" }).Append(
        ...
        div({
            id: "main-wrapper",
            className: `${routeExists ? "ml-20" : ""} print:ml-0 flex-1 h-screen overflow-y-auto`,
        })
    );
}

function createSidebar(_user, onRouteChange) {
    return nav({
        className:
            "fixed top-0 left-0 h-screen w-20 bg-gray-900 text-white flex flex-col items-center py-6 space-y-4 z-40 no-print",
    }).Append(
        a({
            id: "-nav",
            href: "/",
...
function createNavItem(iconName, label, colors, route, onRouteChange) {
    return a({
        ...
        RichElement("i", {
            className: "w-6 h-6",
            dataset: { lucide: iconName },
        }),
        span({
            className:
                "sidebar-tooltip absolute left-full ml-4 px-3 py-1.5 bg-gray-700 text-white text-sm rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none",
            textContent: label,
        })
    );
}
